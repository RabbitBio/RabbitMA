...
class HashTableST {
 public:
  ...
  // Probe with the key itself and materialize a Value only on a miss.  The
  // local assembler sees the same k-mer many times; constructing and copying
  // a full HashGraphVertex for every successful lookup needlessly moves its
  // large fixed-width key through the hot loop.
  reference find_or_insert_key(const key_type &key) {
    rehash_if_needed(size_);

    const uint64_t hash_value = hash_key(key);
    const uint64_t index = bucket_index(hash_value);

    for (node_type *node = buckets_[index]; node; node = node->next) {
      if (key_equal_(get_key_(node->value), key)) {
        return node->value;
      }
    }

    node_type *p = pool_.construct();
    p->value = value_type(key);
    p->next = buckets_[index];
    buckets_[index] = p;
    ++size_;
