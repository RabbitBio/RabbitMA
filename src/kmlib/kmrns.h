...
class RankAndSelect {
 public:
  ...
  void from_packed_array(const word_type *packed_array, size_type size) {
    ...
    const int first_char = BaseSize == 1 ? 1 : 0;
#pragma omp parallel for schedule(static) if (kAlphabetSize - first_char > 1)
    for (int char_index = first_char;
         char_index < static_cast<int>(kAlphabetSize); ++char_index) {
      const unsigned c = static_cast<unsigned>(char_index);
      ...
      l1_occ_[c][num_l1 - 1] =
          count - l2_occ_[c][(num_l1 - 1) / kL1PerL2];
      char_count_[c] = count;
