...
class UnitigGraphVertex {
 public:
  ...
  // The historical constructor scans terminal SDBG edges in ascending order.
  // Keep that order available to memory-saving builders that discover paths
  // from their starts instead.
  uint64_t TerminalOrderKey() const {
    return std::min<uint64_t>(strand_info[0].end, strand_info[1].end);
  }
