...
struct SdbgBucketRecord {
  ...
  // Runtime-only prefix into the compact large-multiplicity value stream.
  // It is recomputed from num_large_mul while loading metadata and therefore
  // does not change the on-disk metadata format.
  size_t accumulate_large_mul_count{0};
