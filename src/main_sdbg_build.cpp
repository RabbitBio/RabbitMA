...
void ValidateResourceOptions(double host_mem, int num_threads) {
  // BaseSequenceSortingEngine stores the memory budget as int64_t.  Reject
  // values that cannot be converted before constructing a sorting engine;
  // converting NaN, infinity, or an out-of-range double is undefined.
  const double int64_limit =
      static_cast<double>(std::numeric_limits<int64_t>::max());
  if (!std::isfinite(host_mem) || host_mem <= 0 || host_mem >= int64_limit) {
    throw std::logic_error(
        "Host memory must be a positive finite value smaller than INT64_MAX!");
  }
  if (num_threads <= 0) {
    throw std::logic_error("Number of CPU threads must be greater than 0!");
  }
}
...
int main_kmer_count(int argc, char **argv) {
  ...
  desc.AddOption("mem_flag", "", opt.mem_flag,
                 ...
                 "balance passes and cache footprint for the thread count; "
                 "other: use all "
  ...
  try {
    ...
    ValidateResourceOptions(opt.host_mem, opt.n_threads);
