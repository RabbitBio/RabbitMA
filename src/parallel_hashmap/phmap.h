...
public:
    ...
    // Read-only description of the single allocation containing control
    // bytes and slots. Callers may use it for advisory NUMA/huge-page policy;
    // ownership and table layout remain private to the container.
    void* storage_address() {
        return capacity_ == 0 ? nullptr : static_cast<void*>(ctrl_);
    }
    size_t storage_bytes() const {
        return capacity_ == 0 ? 0u : MakeLayout(capacity_).AllocSize();
    }
