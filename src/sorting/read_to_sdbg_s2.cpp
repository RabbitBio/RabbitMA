...
void Read2SdbgS2::Lv0CalcBucketSizeFor(
  ...
  for (int64_t read_id = seq_from; read_id < seq_to; ++read_id) {
    ...
    edge.InitFromPtr(read_p, offset, edge_length);
    rev_edge = edge;
    ...
    while (true) {
      if (for_sure_solid || seq_pkg_->is_solid.at(full_offset)) {
        bool is_palindrome = rev_edge.cmp(edge, edge_length) == 0;
...
void Read2SdbgS2::Lv0CalcBucketSize(
    int64_t seq_from, int64_t seq_to,
    std::array<int64_t, kNumBuckets> *out) {
...
template <unsigned NWords>
void Read2SdbgS2::Lv1FillOffsetsFor(OffsetFiller &filler, int64_t seq_from,
                                    int64_t seq_to) {
  const unsigned edge_length = opt_.k + 1;
  const unsigned max_edge_length = Kmer<NWords, uint32_t>::max_size();
  if (UNLIKELY(edge_length == 0 || edge_length > max_edge_length)) {
...
#define CHECK_AND_SAVE_OFFSET(strand, edge_type)                        \
  do {                                                                  \
    if (filler.IsHandling(key)) {                                       \
      filler.WriteNextOffset(                                           \
          key, encoded_offset | ((edge_type) << 1u) | (strand));        \
    }                                                                   \
  } while (0)
    // =========== end macro ==========================
    ...
    unsigned last_char_offset = opt_.k;
    int64_t full_offset = seq_view.full_offset_in_pkg();
    int64_t encoded_offset = full_offset << 3u;
    bool for_sure_solid = opt_.solid_threshold == 1;
    ...
    while (true) {
      if (for_sure_solid || seq_pkg_->is_solid.at(full_offset)) {
        ...
        if (last_char_offset == read_length - 1 ||
          ...
          key = (edge.data()[0] << 4) >>
                (kCharsPerEdgeWord - kBucketPrefixLength) * kBitsPerEdgeChar;
          CHECK_AND_SAVE_OFFSET(0, 2);

          if (!is_palindrome) {
            key = rev_edge.data()[0] >>
                  (kCharsPerEdgeWord - kBucketPrefixLength) * kBitsPerEdgeChar;
            CHECK_AND_SAVE_OFFSET(1, 2);
          }
        }
      }
