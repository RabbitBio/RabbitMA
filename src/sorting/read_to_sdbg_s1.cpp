...
void Read2SdbgS1::Lv0CalcBucketSizeFor(
    int64_t seq_from, int64_t seq_to,
    std::array<int64_t, kNumBuckets> *out) {
  const unsigned kmer_length = opt_.k - 1;
  const unsigned max_kmer_length = Kmer<NWords, uint32_t>::max_size();
  if (UNLIKELY(kmer_length == 0 || kmer_length > max_kmer_length)) {
    xfatal("Invalid k-mer length: {}\n", kmer_length);
  }
  auto &bucket_sizes = *out;
  std::fill(bucket_sizes.begin(), bucket_sizes.end(), 0);
  Kmer<NWords, uint32_t> k_minus1_mer, rev_k_minus1_mer;
  ...
  for (int64_t read_id = seq_from; read_id < seq_to; ++read_id) {
    ...
    k_minus1_mer.InitFromPtr(read_p, offset, kmer_length);
    rev_k_minus1_mer = k_minus1_mer;
    rev_k_minus1_mer.ReverseComplement(kmer_length);
    ...
    k_minus1_mer.ShiftAppend(c, kmer_length);
    rev_k_minus1_mer.ShiftPreappend(3 - c, kmer_length);

    while (last_char_offset < read_length - 1) {
      int cmp = k_minus1_mer.cmp(rev_k_minus1_mer, kmer_length);
...
void Read2SdbgS1::Lv0CalcBucketSize(
    int64_t seq_from, int64_t seq_to,
    std::array<int64_t, kNumBuckets> *out) {
#define DISPATCH_KMER_WORDS(N)                         \
  case N:                                              \
    return Lv0CalcBucketSizeFor<N>(seq_from, seq_to, out)
  switch (DivCeiling(opt_.k - 1, kCharsPerEdgeWord)) {
...
template <unsigned NWords>
void Read2SdbgS1::Lv1FillOffsetsFor(OffsetFiller &filler, int64_t seq_from,
                                    int64_t seq_to) {
  const unsigned kmer_length = opt_.k - 1;
  const unsigned max_kmer_length = Kmer<NWords, uint32_t>::max_size();
  if (UNLIKELY(kmer_length == 0 || kmer_length > max_kmer_length)) {
...
#define CHECK_AND_SAVE_OFFSET(strand)                        \
  do {                                                       \
...
#define DISPATCH_KMER_WORDS(N)                                  \
  case N:                                                       \
    return Lv1FillOffsetsFor<N>(filler, seq_from, seq_to)
  switch (DivCeiling(opt_.k - 1, kCharsPerEdgeWord)) {
    DISPATCH_KMER_WORDS(1);
    DISPATCH_KMER_WORDS(2);
    DISPATCH_KMER_WORDS(3);
