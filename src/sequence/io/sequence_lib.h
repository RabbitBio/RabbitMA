...
class SequenceLib {
 public:
  ...
  int64_t global_begin() const { return held_begin_; }
  int64_t global_end() const { return held_end_; }
...
class SequenceLibCollection {
 public:
  ...
  // Load library boundaries and pairing metadata without materializing the
  // packed reads.  Local mapping can then consume the indexed binary stream
  // directly and keep memory proportional to active chunks.
  void ReadMetadata(SeqPackage *data_holder = nullptr);
